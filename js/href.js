


window.CATALOGS = window.CATALOGS || {};

window.CATALOGS.href = {
    title: "🪲 Проекты",

    items: [
        {
            name: "Калькулятор акустики помещений",
            img: "IMG_proj/calc.jpg",
            desc: `
      тут разные расчёты
      <br>
        <a href="https://aleksei-demon.github.io/calc_2.0/" target="_blank">
            🚀 Открыть калькулятор
        </a>
            `
        },
        {
            name: "Покер",
            img: "IMG_proj/pok.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/Poker/" target="_blank">
          🚀 Открыть игру в покер
      </a>
            `
        },

        {
            name: "число словами",
            img: "IMG_proj/numV.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/numVoice/" target="_blank">
          🚀 Открыть числа словами
      </a>
            `
        },
        {
            name: "Хоррор",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/horr/" target="_blank">
          🚀 Открыть хоррор
      </a>
            `
        },

        {
            name: "Викторина",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/vic/" target="_blank">
          🚀 Открыть викторину
      </a>
            `
        },

        {
            name: "Космос",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/kosm/" target="_blank">
          🚀 Открыть космос
      </a>
            `
        },

        {
            name: "X-zerO",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/X-zerO/" target="_blank">
          🚀 Открыть X-zerO
      </a>
            `
        },

        {
            name: "Новый Год",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/newy/" target="_blank">
          🚀 Открыть открытку
      </a>
            `
        },

        {
            name: "77 лет",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/77years/" target="_blank">
          🚀 Открыть 77 лет
      </a>
            `
        },

        {
            name: "игра в дурака",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/full/" target="_blank">
          🚀 Открыть игру в дурака
      </a>
            `
        },

        {
            name: "Куб",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/three/" target="_blank">
          🚀 Открыть КУБ
      </a>
            `
        },

        {
            name: "Тетрис",
            img: "IMG_proj/.jpg",
            desc: `      
      <br><br>
      <a href="https://aleksei-demon.github.io/demc/" target="_blank">
          🚀 Открыть тетрис
      </a>
            `
        },







    ]
}